...
LOG_FILE="$SCRIPT_DIR/Actions_Logs/AC03_Master-Left.log"
LAST_STATE_DIR="$ROOT_DIR/Actions/Actions_Last-Valid-States"
LAST_STATE_FILE="$LAST_STATE_DIR/AC03_Master-Left.last-valid-state"
...
# ===== SAFE WRITERS =====
safe_write() { # safe_write <file> <text...>
    local _file=$1; shift
    local _txt="$*"
    mkdir -p "$(dirname "$_file")"
    printf '%s\n' "$_txt" >> "$_file"
}
safe_write_no_nl() { # append text without newline
    local _file=$1; shift
    local _txt="$*"
    mkdir -p "$(dirname "$_file")"
    printf '%s' "$_txt" >> "$_file"
...
: > "$LOG_FILE"
log "INFO: Script started (v2.6 SafeWriter)."
...
TMP_PAYLOAD="$(mktemp "${LAST_STATE_DIR}/AC03_payload.XXXXXX")" || { log "ERROR: mktemp failed"; exit 1; }
...
for ((m_id=1; m_id<=${MONITOR_COUNT:-0}; m_id++)); do
    ...
    if wmctrl -i -r "$MASTER_WID" -e 0,"$SAFE_X","$SAFE_Y","$TARGET_MASTER_W","$TARGET_MASTER_H"; then
        log "INFO: Master $MASTER_WID applied ${TARGET_MASTER_W}x${TARGET_MASTER_H} at ${SAFE_X},${SAFE_Y}"
    else
        log "WARN: wmctrl failed applying master $MASTER_WID"
    fi
    ...
    if [ "$COUNT" -gt 1 ]; then
        ...
        STACK_AVAIL_W=$(( LIMIT_RIGHT - STACK_START_X ))
        if [ "$STACK_AVAIL_W" -le 0 ]; then STACK_AVAIL_W=120; fi
        ...
        for ((idx=1; idx<COUNT; idx++)); do
            ...
            if wmctrl -i -r "$WID" -e 0,"$POS_X","$POS_Y","$TARGET_STACK_W","$TARGET_STACK_H"; then
                ...
                log "WARN: wmctrl failed applying stack $WID"
            fi
...
# ===== FINALIZE & WRITE LAST STATE =====
if [ "$TOTAL_TILED" -gt 0 ]; then
    printf '✅ AC03: Master-Left applied to %d window(s).\n' "$TOTAL_TILED"
else
    printf '⚠️ AC03: No windows found on active desktop.\n'
fi

FINAL_TMP="$(mktemp "${LAST_STATE_DIR}/AC03_payload.final.XXXXXX")" || { log "ERROR: mktemp final failed"; exit 1; }
