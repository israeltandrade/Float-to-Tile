#!/usr/bin/env bash
# File: AC10_Focus-Next-Monitor.sh
# Description: v1.1 - Switch focus to the first window on the next monitor (per desktop) when multiple monitors present.
# Notes:
# - Loads Screen/Monitor/Window data (prefers *.data, falls back to *.last-valid-state).
# - Writes last-valid-state and logs similar to AC08/AC09.
...
LOG_FILE="$SCRIPT_DIR/Actions_Logs/AC10_Focus-Next-Monitor.log"
...
LAST_STATE_FILE="$LAST_STATE_DIR/AC10_Focus-Next-Monitor.last-valid-state"
...
write_last_state() {
  ...
  tmp="$(mktemp "${LAST_STATE_DIR}/AC10_payload.XXXXXX")" || return 1
  printf '%s\n' "$payload" > "$tmp"
  final="$(mktemp "${LAST_STATE_DIR}/AC10_payload.final.XXXXXX")" || { rm -f "$tmp"; return 1; }
  mv -f "$tmp" "$final"
...
# If any essential missing, try run.sh
...
if [[ -z "$CURRENT_WID" || "$CURRENT_WID" == "0x0" ]]; then
  ...
  printf "⚠️ AC10 SKIPPED: Current WID invalid (0x0).\n"; exit 0
fi
...
if (( MON_COUNT < 2 )); then
  ...
  printf "⚠️ AC10 SKIPPED: single-monitor system.\n"; exit 0
fi

# Compute next monitor (wrap-around)
CUR_MON_IDX=$((CURRENT_MONITOR + 0))
NEXT_MON_IDX=$(( (CUR_MON_IDX % MON_COUNT) + 1 ))
log "INFO" "Current monitor $CUR_MON_IDX -> next monitor $NEXT_MON_IDX (MONITOR_COUNT=$MON_COUNT)"
...
# Find first window in target group D{desktop}_M{next}_CYCLE_LIST (strictly same desktop)
GROUP_VAR="D${CUR_DESKTOP}_M${NEXT_MON_IDX}_CYCLE_LIST"
GROUP_VAL="${!GROUP_VAR:-}"

if [[ -z "$GROUP_VAL" ]]; then
  log "INFO" "No cycle list for desktop ${CUR_DESKTOP} monitor ${NEXT_MON_IDX} (var $GROUP_VAR empty)."
  printf "⚠️ AC10 SKIPPED: No windows on monitor %s for desktop %s.\n" "$NEXT_MON_IDX" "$CUR_DESKTOP"
  exit 0
fi
...
if ((${#arr_group[@]} == 0)); then
  ...
  printf "⚠️ AC10 SKIPPED: No windows in group %s.\n" "$GROUP_VAR"
  exit 0
fi

TARGET_WID="${arr_group[0]}"
TARGET_WID="$(normalize_wid "$TARGET_WID")"
log "INFO" "Candidate target first window on D${CUR_DESKTOP}_M${NEXT_MON_IDX} is $TARGET_WID"

# === NEW SAFETY CHECK: ensure TARGET_WID belongs to the SAME desktop ===
...
if [[ -z "$TARGET_DESKTOP" ]]; then
  ...
  printf "⚠️ AC10 SKIPPED: Cannot verify target desktop for %s.\n" "$TARGET_WID"
  exit 0
fi
...
if [[ "$TARGET_DESKTOP" != "$CUR_DESKTOP" ]]; then
  ...
  printf "⚠️ AC10 SKIPPED: Target window is on a different desktop (%s) — refusing to switch desktops.\n" "$TARGET_DESKTOP"
  exit 0
fi
...
if command -v wmctrl &>/dev/null; then
  ...
  printf "❌ AC10 FAILED: wmctrl not available.\n"; exit 1
fi
...
payload="$(cat <<EOF
### AC10 Last valid state
TIMESTAMP=${TIMESTAMP}
DESKTOP=${CUR_DESKTOP}
CURRENT_MONITOR=${CUR_MON_IDX}
NEXT_MONITOR=${NEXT_MON_IDX}
...
printf "✅ AC10 SUCCESS: Focused first window on monitor %s: %s (Desktop %s).\n" "$NEXT_MON_IDX" "$TARGET_WID" "$CUR_DESKTOP"
exit 0
